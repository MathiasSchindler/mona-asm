...
static void die_line(const char *msg, const char *line) {
...
static void symvec_push(SymVec *v, Symbol s) {
    if (v->len + 1 > v->cap) {
...
static void relocvec_push(RelocVec *v, Reloc r) {
    if (v->len + 1 > v->cap) {
        v->cap = v->cap ? v->cap * 2 : 32;
...
static void linevec_push(LineVec *v, char *line) {
    if (v->len + 1 > v->cap) {
...
static void read_lines_recursive(const char *path, LineVec *out) {
    FILE *f = fopen(path, "r");
    if (!f) {
...
static int sym_find(SymVec *syms, const char *name) {
...
static int split_operands(char *s, char **ops, int max_ops) {
    int depth = 0;
    int count = 0;
    char *start = s;
    for (char *p = s; ; p++) {
...
static int parse_mem(const char *s, Mem *out) {
...
static int parse_operand(const char *s, Operand *op) {
...
static void emit_u8(Buf *out, uint8_t v) {
    buf_append(out, &v, 1);
}
...
static void emit_ascii(Buf *out, const char *line) {
...
static void emit_text_line(const char *line, Buf *out, SymVec *syms, RelocVec *relocs, Section sec) {
...
static void pass1(LineVec *lines, SymVec *syms, uint32_t *text_size, uint32_t *data_size, uint32_t *bss_size) {
...
static void pass2(LineVec *lines, SymVec *syms, Buf *text, Buf *data, uint32_t *bss_size, RelocVec *relocs) {
    Section sec = SEC_NONE;
    uint32_t bsize = 0;
...
static void write_mobj(const char *path, Buf *text, Buf *data, uint32_t bss_size, SymVec *syms, RelocVec *relocs) {
