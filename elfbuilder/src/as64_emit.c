...
void emit_text_line(const char *line, Buf *out, SymVec *syms, RelocVec *relocs, Section sec) {
    ...
    if (strcmp(mn, "mov") == 0 || strcmp(mn, "movb") == 0 || strcmp(mn, "movq") == 0 || strcmp(mn, "movl") == 0) {
        int size = 0;
        if (strcmp(mn, "movb") == 0) size = 8;
        if (strcmp(mn, "movq") == 0) size = 64;
        if (strcmp(mn, "movl") == 0) size = 32;
    ...
    if (strcmp(mn, "cmpb") == 0) {
        if (o1.kind == OP_IMM && (o2.kind == OP_REG || o2.kind == OP_MEM)) {
            if (o1.imm == 0 && o2.kind == OP_REG) {
                emit_op_reg_rm(out, 0x84, &o2.reg, &o2, 0, relocs, syms, sec, 0);
                free(work);
                return;
            }
            check_imm8(o1.imm);
    ...
    if (strcmp(mn, "add") == 0) {
        if (o1.kind == OP_IMM && o2.kind == OP_REG) {
            if (o1.imm == -1) {
                if (o2.reg.size == 8) {
                    emit_op_digit_rm(out, 0xFE, 1, &o2, 0, relocs, syms, sec, 0);
                } else {
                    emit_op_digit_rm(out, 0xFF, 1, &o2, o2.reg.size == 64, relocs, syms, sec, 0);
                }
                free(work);
                return;
            }
            if (o1.imm == 1) {
                if (o2.reg.size == 8) {
                    emit_op_digit_rm(out, 0xFE, 0, &o2, 0, relocs, syms, sec, 0);
                } else {
                    emit_op_digit_rm(out, 0xFF, 0, &o2, o2.reg.size == 64, relocs, syms, sec, 0);
                }
                free(work);
                return;
            }
    ...
    if (strcmp(mn, "and") == 0) {
        if (o1.kind == OP_IMM && o2.kind == OP_REG) {
            if (o1.imm == 0) {
                uint8_t op = (o2.reg.size == 8) ? 0x30 : 0x31;
                emit_op_reg_rm(out, op, &o2.reg, &o2, 0, relocs, syms, sec, 0);
                free(work);
                return;
            }
            if (o1.imm == -1) {
